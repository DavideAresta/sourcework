...
- **Writes require the `X-PRDForge-UI: 1` header.** Not a secret — its only job
...
only looks at the first URL. `PRDFORGE_SECURITY__ALLOW_PRIVATE_FETCH=1` exists
...
PRDFORGE_SECURITY__API_KEY=$(python -c 'import secrets; print(secrets.token_urlsafe(32))')
```
