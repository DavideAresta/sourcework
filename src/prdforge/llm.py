...
* **A coding CLI** - ``claude``, ``opencode`` or ``copilot`` - driven as a
  ...
  implementations live in :mod:`prdforge.backends`.
...
* **Failover.** When the active backend reports a usage limit - or exits
  ...
  ``PRDFORGE_LLM__FAILOVER_ORDER`` rather than failing. The model does *not*
...
from prdforge import usage as usage_ledger
from prdforge.backends import build, resolve_chain
...
from prdforge.config import LLMSettings, effective_llm
...
class LLM:
    ...
    def cfg(self) -> LLMSettings:
        ...
        per-run overrides (:func:`prdforge.config.llm_overrides`) invisible to
