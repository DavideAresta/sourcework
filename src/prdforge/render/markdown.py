...
from prdforge.models import PRDDocument, ReviewReport
...
def render(prd: PRDDocument, review: ReviewReport | None = None) -> str:
    ...
    L.append(
        ...
        f"**Generated** {prd.generated_at:%Y-%m-%d %H:%M UTC} by PRD Forge"
    )
    L.append(">")
    L.append(
    ...
    for kind in ("functional", "non_functional", "constraint", "assumption", "out_of_scope"):
        ...
        L += ["| ID | Pri | Requirement | Acceptance criteria | Sources |", "|---|---|---|---|---|"]
        for r in group:
            sources = "; ".join(
            ...
            L.append(
                f"| {r.id} | {r.priority.value.upper()} | **{_esc(r.title)}**{tag}<br>"
    ...
    if prd.milestones:
        L += ["## Milestones", "", "| Milestone | Requirements | Target |", "|---|---|---|"]
        for m in prd.milestones:
            L.append(
                f"| **{_esc(m.name)}** — {_esc(m.description or '')} | "
                f"{', '.join(m.requirement_ids)} | {_esc(m.target or '—')} |"
            )
        L.append("")
