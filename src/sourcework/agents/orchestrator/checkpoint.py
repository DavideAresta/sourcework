...
def saved_stages(run_id: str | None) -> list[str]:
    """Which stages ``run_id`` has on disk, in the order they were produced.
    ...
    """
    stored = Checkpoint(run_id=run_id)._read().get("stages")  # noqa: SLF001 - same module
    return list(stored) if isinstance(stored, dict) else []


def discard(run_id: str | None) -> None:
    """Forget a run's saved stages. Called when the run itself is deleted."""
    Checkpoint(run_id=run_id).clear()
...
class Checkpoint:
    ...
    def path(self) -> Path | None:
        return directory() / f"{self.run_id}.json" if self.run_id else None
    ...
    def _read(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("checkpoint %s: unreadable, starting fresh", self.run_id)
            return {}
