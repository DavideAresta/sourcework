...
SourceWork addresses models by id and reaches them over one OpenAI-compatible
...
* **Directories are configuration, not code.** ``SOURCEWORK_MODEL_DIRS`` lists
...
def swap_config(
    ...
    *,
    ...
    Generated rather than hand-written so that the id SourceWork asks for and
    the file llama-server opens cannot drift apart.
    """
    lines = [
        "# Generated by sourcework-models. Edits here are lost on the next scan -",
        "# add a directory to SOURCEWORK_MODEL_DIRS, or edit llama-swap.yaml instead.",
